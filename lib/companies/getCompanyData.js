...
export async function getCompanyData({ country, company }) {
  ...
  const response = {
    ...
    section3: hasBuyerSupplierData && hasShipmentData
      ? {
          buyers: Number(ubs[0]?.Buyers) || 0,
          suppliers: Number(ubs[1]?.Suppliers) || 0,
          shipmentSent:Number(vc[0]?.["Total Export Shipment"]) || 0,
          shipmentReceived:Number(vc[1]?.["Total Import Shipment"]) || 0,
        }
      : {
          buyers: 0,
          suppliers: 0,
  ...
  !hasShipmentData &&
  !hasBuyerSupplierData &&
  response.section4.import.hsCodes.length === 0 &&
  response.section4.export.hsCodes.length === 0 &&
  response.section4.import.countries.length === 0 &&
  response.section4.export.countries.length === 0;
